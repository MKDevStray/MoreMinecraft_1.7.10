...
public class EntityCustomWM extends EntityWaterMob
{
    ...
    public boolean gettingOutOfWater()
    {
        ...
        int k = (int)posZ;
        int l = 1;
        l = worldObj.getBlockMetadata(i, j + 1, k);
        return l == 0;
    }
    ...
    public void moveEntityWithHeading(float f, float f1)
    {
        ...
        float f4 = 0.91F;
        f4 = 0.5460001F;
        int i = worldObj.getBlockMetadata(MathHelper.floor_double(posX), MathHelper.floor_double(boundingBox.minY) - 1, MathHelper.floor_double(posZ));
        if(i > 0)
        {
            f4 = Block.getBlockById(i).slipperiness * 0.91F;
        }
        float f5 = 0.162771F / (f4 * f4 * f4);
        moveFlying(f, f1, 0.1F * f5);
        f4 = 0.91F;
        f4 = 0.5460001F;
        int j = worldObj.getBlockMetadata(MathHelper.floor_double(posX), MathHelper.floor_double(boundingBox.minY) - 1, MathHelper.floor_double(posZ));
        if(j > 0)
        {
            f4 = Block.getBlockById(j).slipperiness * 0.91F;
        }
        if(isOnLadder())
        {
            fallDistance = 0.0F;
...
//            }
      ...
      //  {
       ...
       // }
        ...
        //{
            ...
            {
                int m1 = MathHelper.floor_double((posX + (double)rand.nextInt(13)) - 6D);
                ...
                float f3 = getBlockPathWeight(m1, k1, l1);
                if(f3 > f2)
                {
                    f2 = f3;
                    m = m1;
          ...
          // if(flag)
          //  {
          //      a = worldObj.getEntityPathToXYZ(this, j, k, l, 10F, flag, flag, flag, flag);
          //  }
       // }
        int o = MathHelper.floor_double(boundingBox.minY);
  ...
  //          float f5 = f4 - rotationYaw;
         ...
         //   for(; f5 >= 180F; f5 -= 360F) { }
          ...
          //  {
           ...
           // {
            ...
            //}
           // if(d3 > 0.0D && player != null && EntityHelper.getInWater(playerToAttack))
            //{
            //    isJumping = true;
           // }
       // }
      ...
      //      {
        //        continue;
         //   }
           ...
           // if(entityitem1.getEntityItem() != Items.fish.shiftedIndex || !EntityHelper.getInWater(entityitem1))
           // {
            //    continue;
           // }
        ...
        public void onLivingUpdate()
    {
        ...
        if(this.getHealth() <= 0 || !inWater && riddenByEntity == null)
        {
            isJumping = false;
            moveStrafing = 0.0F;
            moveForward = 0.0F;
