...
public class ItemLawLauncher extends Item { 

	public ItemLawLauncher(Item goldIngot) {
		this.setFull3D();
	 ...
	 public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer)
	   {
	    	   if(par3EntityPlayer.inventory.hasItem(ModMain.lawRocket)) {
	           //--par1ItemStack.stackSize;
	    		   if(!par3EntityPlayer.capabilities.isCreativeMode) {
	    		   par3EntityPlayer.inventory.consumeInventoryItem(ModMain.lawRocket);
	    		   }
...
//		    	   double posZ = par3EntityPlayer.getLookVec().zCoord;
	       par2World.playSoundAtEntity(par3EntityPlayer, "mi:RocketFire1", 0.5F, 1.0F);// / (itemRand.nextFloat() * 0.4F + 0.8F));
//	       par2World.spawnParticle("flame", posX, posY, posZ, 0.0D, 0.0D, 0.0D);
	       if (!par2World.isRemote)
	       {
	           par2World.spawnEntityInWorld(new EntityM67Grenade(par2World, par3EntityPlayer));
